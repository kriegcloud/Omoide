...
Inputs are at most 16 MiB compressed and 16 million decoded pixels. EXIF orientation applies once; metadata is stripped; the final derivative is PNG (`compress_level=9`, `optimize=False`), at most 2 MiB; no crop or resize occurs. The transform is `rgb-png-v2`, whose cache/provenance key carries the Pillow/codec versions, the materializer and media-module implementation SHA-256 digests, the requested frame and, for video, the decoder identity. Export manifests separately pin the loaded authority/materialization/media/planning/export source-module SHA-256 digests. Code/profile version changes must bump the transform/policy version; exact file identity is not promised across decoder/encoder versions.
